...
async function startContainer(
  ...
  let manifest: {
    runtime?: { port?: number; command?: string[] };
    route?: string;
  };
  try {
  ...
  const securityFlags =
    appType === "system" ? SYSTEM_APP_SECURITY_FLAGS : USER_APP_SECURITY_FLAGS;
  ...
  const args = [
    ...
    `APP_ID=${appId}`,
    "-e",
    `PORT=${manifest.runtime?.port || 3000}`,
  ];
  ...
  if (portMappings && portMappings.length > 0) {
    for (const mapping of portMappings) {
      if (mapping.enabled) {
        ...
        log(
          "info",
          `Mapping port ${mapping.hostPort}:${mapping.containerPort}/${mapping.protocol}`,
        );
      }
    }
  }
  ...
  socket.on("data", async (data: Buffer) => {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        log(
          "debug",
          `Request: ${request.op} ${"appId" in request ? request.appId : ""}`,
        );
